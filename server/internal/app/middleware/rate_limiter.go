...
//		_ = res.AddError(err, handlers.KNOWN_ERROR, []string{"rate_limiter.go", "While adding timestamp to Redis"})
//		return
//	}
//
...
//	if err != nil {
//		res.Status(http.StatusInternalServerError)
//		_ = res.AddError(err, handlers.KNOWN_ERROR, []string{"rate_limiter.go", "While counting timestamps in Redis"})
//		return
//	}
//
//	if numRequests > rateLimit {
//		res.Status(http.StatusTooManyRequests)
//		_ = res.AddError(fmt.Errorf("rate limit exceeded"), handlers.RATE_LIMIT_EXCEEDED, []string{"rate_limiter.go", "Rate limit exceeded"})
//		return
//	}
//}
